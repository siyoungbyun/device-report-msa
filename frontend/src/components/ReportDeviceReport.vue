<template>
    <v-card outlined>
        ...
        <v-card-title v-if="value._links">
            단말기신고 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            단말기신고
        </v-card-title >
        ...
        <v-card-text style="background-color: white;">
            ...
            <String label="단말기ID" v-model="value.deviceId" :editMode="editMode" :inputUI="''"/>
            ...
            <Status offline label="신고상태" v-model="value.status" :editMode="editMode" @change="change"/>
        </v-card-text>
        ...
        <v-card-actions>
            ...
            <v-dialog v-model="reviewDiagram" width="500">
                <ReviewCommand
	...
	async created() {
        ...
        methods: {
            ...
            async review(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['review'].href), params)
            ...
            openReview() {
                this.reviewDiagram = true;
            },
            closeReview() {
                this.reviewDiagram = false;
            },
        },
    }
</script>
